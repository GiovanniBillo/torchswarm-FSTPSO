...
class Frbs:
    """
    Vectorized fuzzy rule-based system (FRBS).
    delta: (N,)
    phi:   (N,)
    outputs: each (N,)
    """
    ...
    def __init__(self, delta_max, verbose=False):
        # delta_max can be float or (N,)
        if isinstance(delta_max, (int, float)):
            self.delta_max = torch.tensor(float(delta_max))
        else:
            self.delta_max = delta_max.float()

        self.verbose = verbose

        # thresholds (broadcastable)
        ...
        z_list = [
            torch.tensor([0.3, 0.3, 0.5, 0.5, 1.0, 1.0], dtype=torch.float32),        # Inertia (len 6)
            torch.tensor([1.0, 1.0, 2.0, 2.0, 3.0, 3.0], dtype=torch.float32),        # Social  (len 6)
            torch.tensor([0.1, 1.5, 1.5, 1.5, 1.5, 3.0], dtype=torch.float32),        # Cognitive (len 6)
            torch.tensor([0.0, 0.0, 0.0, 0.001, 0.001, 0.01], dtype=torch.float32),   # L (len 6)
            torch.tensor([0.1, 0.15, 0.15, 0.15, 0.15, 0.2], dtype=torch.float32),    # U (len 6)
        ]
        self.z_dict = {name: z_list[i] for i, name in enumerate(self.components)}

        print("Initialized Fuzzy Rule Based System (vectorized)")

    # ------------------------
    #   Memberships (vectorized)
    # ------------------------
    def delta_memberships(self, delta: torch.Tensor):
        """
        delta: (N,)
        returns dict with keys Same/Near/Far each (N,)
        """
        assert delta.ndim == 1, f"delta must be (N,), got {delta.shape}"
        device = delta.device
        dtype = delta.dtype

        # broadcast thresholds to (N,)
        d1 = self.d1.to(device=device, dtype=dtype)
        d2 = self.d2.to(device=device, dtype=dtype)
        d3 = self.d3.to(device=device, dtype=dtype)
        dmax = self.delta_max.to(device=device, dtype=dtype)

        # clamp delta into [0, dmax] to avoid weirdness
        delta = torch.clamp(delta, 0.0, dmax)
        ...
        # Same:
        # 1 on [0,d1)
        # linearly down on [d1,d2)
        # 0 on [d2, dmax]
        same = torch.where(delta < d1, one, zero)
        same = torch.where((delta >= d1) & (delta < d2),
                           (d2 - delta) / torch.clamp(d2 - d1, min=1e-12),
                           same)
        same = torch.where(delta >= d2, zero, same)
        ...
        # down on [d2,d3)
        # 0 on [d3, dmax]
        near = torch.where(delta < d1, zero, zero)
        near = torch.where((delta >= d1) & (delta < d2),
                           (delta - d1) / torch.clamp(d2 - d1, min=1e-12),
                           near)
        near = torch.where((delta >= d2) & (delta < d3),
                           (d3 - delta) / torch.clamp(d3 - d2, min=1e-12),
                           near)
        near = torch.where(delta >= d3, zero, near)

        # Far:
        # 0 on [0,d2)
        # up on [d2,d3)
        # 1 on [d3,dmax]
        far = torch.where(delta < d2, zero, zero)
        far = torch.where((delta >= d2) & (delta < d3),
                          (delta - d2) / torch.clamp(d3 - d2, min=1e-12),
                          far)
        far = torch.where(delta >= d3, one, far)

        return {"Same": same, "Near": near, "Far": far}

    def phi_memberships(self, phi: torch.Tensor):
        """
        phi: (N,) expected in [-1,1]
        returns dict Same/Worse/Better each (N,)
        """
        assert phi.ndim == 1, f"phi must be (N,), got {phi.shape}"
        phi = torch.clamp(phi, -1.0, 1.0)

        same = torch.clamp(1.0 - torch.abs(phi), 0.0, 1.0)
        worse = torch.clamp(phi, 0.0, 1.0)
        better = torch.clamp(-phi, 0.0, 1.0)

        return {"Same": same, "Worse": worse, "Better": better}

    # ------------------------
    #   Rules (vectorized)
    # ------------------------
    def define_rules_vectorized(self, delta_m, phi_m):
        """
        delta_m: dict Same/Near/Far -> (N,)
        phi_m:   dict Same/Worse/Better -> (N,)
        returns dict component -> (N,6) rule activations (because you have 6 z values per component)
        """
        SameD, NearD, FarD = delta_m["Same"], delta_m["Near"], delta_m["Far"]
        SameP, WorseP, BetterP = phi_m["Same"], phi_m["Worse"], phi_m["Better"]

        # Build (N,6) for each component to match your z vectors length 6.
        rules = {}

        # Inertia: [phiWorse, deltaSame, phiSame, deltaNear, phiBetter, deltaFar]
        rules["Inertia"] = torch.stack([WorseP, SameD, SameP, NearD, BetterP, FarD], dim=1)

        # Social:  [phiBetter, deltaNear, phiSame, deltaSame, phiWorse, deltaFar]
        rules["Social"]  = torch.stack([BetterP, NearD, SameP, SameD, WorseP, FarD], dim=1)

        # Cognitive (your original had 6 entries too):
        # [deltaFar, phiWorse, phiSame, deltaSame, deltaNear, phiBetter]
        rules["Cognitive"] = torch.stack([FarD, WorseP, SameP, SameD, NearD, BetterP], dim=1)

        # L:
        # [phiSame, phiBetter, deltaFar, deltaSame, deltaNear, phiWorse]
        rules["L"] = torch.stack([SameP, BetterP, FarD, SameD, NearD, WorseP], dim=1)

        # U:
        # [deltaSame, phiSame, phiBetter, deltaNear, phiWorse, deltaFar]
        rules["U"] = torch.stack([SameD, SameP, BetterP, NearD, WorseP, FarD], dim=1)

        return rules

    # ------------------------
    #   Sugeno defuzzification (vectorized)
    # ------------------------
    def sugeno_vectorized(self, rules_dict):
        """
        rules_dict: component -> (N,6)
        returns dict component -> (N,)
        """
        out = {}
        for comp, rules in rules_dict.items():
            # rules: (N,6)
            z = self.z_dict[comp].to(device=rules.device, dtype=rules.dtype)  # (6,)
            num = torch.sum(rules * z[None, :], dim=1)                        # (N,)
            den = torch.sum(rules, dim=1)                                     # (N,)
            out[comp] = torch.where(den > 0, num / den, torch.zeros_like(den))
        return out

    # ------------------------
    #   One-call API (vectorized)
    # ------------------------
    def forward(self, delta: torch.Tensor, phi: torch.Tensor):
        """
        delta, phi: (N,)
        returns dict Inertia/Social/Cognitive/L/U each (N,)
        """
        delta_m = self.delta_memberships(delta)
        phi_m = self.phi_memberships(phi)
        rules = self.define_rules_vectorized(delta_m, phi_m)
        return self.sugeno_vectorized(rules)
